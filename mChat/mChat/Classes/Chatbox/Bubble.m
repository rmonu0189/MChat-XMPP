...
- (id)initWithTextMessage:(NSString *)message isRecieved:(BOOL)recieved UserImage:(UIImage *)image{
    if (self == [super init]) {
        ...
        bubbleImageView = [self setBubbleImageView:CGRectMake(chatTextView.frame.origin.x - 5,chatTextView.frame.origin.y, 0, 0) isRecieved:recieved];
...
- (UIImageView *)setBubbleImageView:(CGRect)frame isRecieved:(BOOL)recieved {
